...
| **Fiche d'un réalisateur ou d'une saga suivis** (brief du 15 septembre 2026, puis marque « introuvable » et généralisation aux sagas le même jour) | Empilée depuis « Suivis », sans barre du bas : une barre de titre avec retour, le nom de l'entité, et un `IconButton` poubelle à droite (`contentDescription` « Ne plus suivre ») qui ouvre un `AlertDialog` à deux boutons avant de retirer l'entité et revenir à la liste. Sous la barre, un interrupteur « Masquer les introuvables » (`Switch`, activé par défaut, mémorisé pour la session, partagé entre les deux sources). Puis les films en `LazyColumn`, de la plus ancienne sortie à la plus récente : l'année (`bodyMedium onSurfaceVariant`), l'affiche 30×45, le titre (`bodyLarge`), et à droite soit la note (`titleMedium`) si le film est vu, soit « introuvable » (`labelMedium onSurfaceVariant`) s'il est marqué, soit une pastille corail « à voir » (`labelMedium onPrimary`, coins entièrement ronds) sur le premier film ni vu ni introuvable, lui seul. Un film marqué introuvable est grisé (opacité 50 %) quand l'interrupteur est désactivé, absent de la liste quand il est activé — la même marque, quelle que soit la fiche où le film apparaît. Toucher un film vu ouvre sa correction (`Screen.Edit`, la même entrée que le journal complet) ; toucher le prochain film non vu ouvre le formulaire pré-rempli. Un appui long sur un film non vu ouvre un `ModalBottomSheet` — « Marquer introuvable » et « Annuler » s'il ne l'est pas encore, « Le remettre à voir » s'il l'est déjà ; l'appel au back fait, la fiche se recharge, un échec affiche « Impossible pour l'instant » en snackbar sans rien changer d'autre. |
