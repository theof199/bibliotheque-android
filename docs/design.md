...
| **Frise** (brief du 15 septembre 2026, devenue le calendrier du siècle le 16) | Quatrième entrée de la `NavigationBar` du bas, entre « Accueil » et « Suivis » — icône `Timeline`, `contentDescription` « Frise » ; sélectionnée sur cet écran. En tête, « Tu en es à *année* → » (`titleLarge`, corail, touchable, ouvre l'année en cours) ou « Tout vu jusqu'ici » (non touchable) ; absente si Seerr n'est pas configuré côté back. Puis une grille sans défilement si elle tient sur l'écran (sinon elle seule défile, la légende et « Sans année » restant fixes dessous) : une ligne par décennie, de la première qui a quelque chose à la décennie courante, décennies intermédiaires vides comprises — l'étiquette (« 1940 », `bodyMedium onSurfaceVariant`, touchable, ouvre `Screen.Decennie`) à gauche, dix cases carrées, une par année. La couleur d'une case suit ses vus (`couleurDeCase`, fonction pure) : fond `surfaceContainerHigh` cerné d'un liseré `outline` à 0 vu, puis quatre teintes montant jusqu'au corail plein à 5 vus et plus. Un liseré intérieur corail de 1,5 dp marque une année qui a au moins un à-voir ; un contour `onSurface` de 2 dp marque l'année en cours ; les années postérieures à aujourd'hui sont invisibles. Une légende sur une ligne sous la grille (« 1 vu », « 5 et plus », « à voir »), puis, s'il y en a, « Sans année · *N* vus · *M* à voir » (`bodyMedium onSurfaceVariant`, touchable). Toucher une case ouvre `Screen.Annee` (`contentDescription` « *année*, *N* vu(s), *M* à voir »). **Le Voyage** (brief du 16 septembre 2026, phase 1) : la phrase de tête vient désormais de `GET /me/voyage` plutôt que du plus ancien à-voir du Plex — « Tu en es à *année* » (touchable, ouvre l'année) ou « *année* se prépare… » (non touchable) selon que sa chronique existe déjà ; absente si le chroniqueur n'est pas configuré côté back (`ANTHROPIC_API_KEY`). Le contour blanc d'une case marque désormais cette même frontière plutôt que le plus ancien à-voir. Une année verrouillée (pas encore atteinte) se grise (opacité désactivée, §6) avec un petit cadenas centré ; une année faite porte une petite étoile en haut à droite. |
| **Décennie** (brief du 16 septembre 2026) | Le rayon d'une décennie, empilé depuis le calendrier de la Frise en touchant l'étiquette d'une décennie, sans barre du bas : une barre de titre avec retour et « Années *décennie* » (`titleLarge`). Puis un anneau de progression (`CircularProgressIndicator`, corail sur `surfaceContainerHigh`) — vus sur vus plus à voir, le pourcentage au centre, « ✓ » à 100 % — à côté de « *N* vus · *M* à voir ». Puis une étagère horizontale (`LazyRow`) des films de la décennie, dans l'ordre des années puis du titre : les vus en tuiles d'affiche notées (52×78 dp, comme l'accueil), les à voir en tuiles au liseré pointillé — celui d'`AnneeScreen`, mais en corail — avec l'année en bas ; toucher un vu ouvre `Screen.Edit`, un à voir `Screen.Form` pré-rempli. Sous l'étagère, dix puces année (`labelSmall`), touchables (`Screen.Annee`), en opacité désactivée (38 %, désactivé du §6) si l'année n'a rien, **ou si le Voyage la déclare verrouillée** (brief du 16 septembre 2026). Retour système : au calendrier. |
| **Année** (brief du 15 septembre 2026, augmentée du Voyage le 16) | Empilé depuis la Frise, sans barre du bas : une barre de titre avec retour et l'année (ou « Sans année »). **Le cartouche du Voyage**, kitsch : papier jauni (`#F2E8D5`) sur le fond noir, cadre ornementé simple, l'année en `titleLarge` — plié (fond `surfaceContainerHigh`, un cadenas, « *N* essentiels t'attendent · *M* vus en avance ») sur une année verrouillée, sans récit ni liste ; déplié sinon, avec le récit et les faits d'une chronique déjà prête, ou « Le chroniqueur écrit… » (indicateur circulaire) tant qu'elle est en préparation — relue toutes les cinq secondes, dix fois au plus (`GET /reference/chroniques/annees/{annee}`) ; une petite étoile à côté du millésime sur une année faite. Puis, si la chronique en porte, « Les essentiels » : une ligne par film (affiche 40×60, rang, titre, réalisateur, le « pourquoi ») et son état à droite — vu (la note), sur le Plex (pastille « sur ton Plex », toucher ouvre le formulaire pré-rempli), à trouver (`TextButton` « Demander sur Sir » → `POST /me/voyage/demander/{tmdbId}`, puis pastille « demandé »), introuvable (grisée, la mention). Un appui long sur un essentiel non vu ouvre le même `ModalBottomSheet` « Marquer introuvable » / « Le remettre à voir » que la fiche d'un réalisateur (`PUT`/`DELETE /me/introuvables/{tmdbId}`, existants). Puis la grille des films vus cette année, mêmes tuiles que l'accueil (note en pastille, toucher ouvre la correction). Puis, si elle n'est pas vide, « À voir sur le Plex » (`titleMedium`) et la même grille pour les films pas encore vus — même affiche, un liseré pointillé `onSurfaceVariant` à la place de la note, jamais de coche ; toucher ouvre le formulaire pré-rempli, comme depuis « Au ciné ». |
