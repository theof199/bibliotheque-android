...
| **Suivis** (brief du 15 septembre 2026 ; sagas ajoutées le même jour, l'écran se nommant jusque-là « Réalisateurs » ; toucher un réalisateur ouvre sa page depuis le brief du 21 septembre 2026, « la page réalisateur », décision 4) | Cinquième entrée de la `NavigationBar` du bas, entre « Frise » et « Au ciné » — icône `Movie` inchangée, `contentDescription` « Suivis » ; sélectionnée sur cet écran. En tête, deux segments (`SingleChoiceSegmentedButtonRow`) « Réalisateurs » et « Sagas », mémorisés pour la session : un seul écran, une seule `LazyColumn`, paramétrée par le segment affiché — jamais deux écrans. **Une carte par entité (`CarteSuivi`, depuis la revue du 24 septembre 2026, point 12 — avant elle, une simple ligne fine)** : `Portrait` (photo ou affiche ronde 48 dp, ou l'initiale sur `surfaceContainerHigh` quand TMDB n'en a pas), le nom (`titleMedium`), puis une barre de progression (`LinearProgressIndicator`, or sur `surfaceContainerHigh`) et « *N* sur *M* » (vus sur total), puis l'affiche (32×48) et le titre du prochain film à voir s'il y en a un — ou « … » tant que `GET .../films` de cette entité n'a pas répondu, ou « indisponible » s'il a échoué, sans bloquer les autres cartes. En haut à droite, un `IconButton` « + » (`contentDescription` « Ajouter un réalisateur » ou « Ajouter une saga », selon le segment) ouvre `Screen.ChercherSuivi`. Toucher une carte d'une saga ouvre sa fiche existante ; toucher une carte d'un réalisateur ouvre désormais `Screen.Realisateur`, sa page (ci-dessous), et non plus cette fiche. Vide (§6 « états vides » — icône Tabler `user` ou `movie`, la phrase, un bouton « Ajouter » depuis la revue du 24 septembre 2026, point 16, qui ouvre la même recherche que le « + » — avant elle, la phrase seule) : « Ajoute un réalisateur avec + » ou « Ajoute une saga avec + ». |
...
| **Réalisateur** (`Screen.Realisateur`, brief du 21 septembre 2026, « la page réalisateur » ; reprise le même jour, « la page réalisateur, reprise », l'étagère horizontale jugée illisible ; retouche du 22 septembre 2026, « la filmographie dans l'ordre ») | Une seule page, suivi ou non, nourrie par `GET /me/realisateurs/{tmdbId}/page` — ouverte depuis « Suivis » ou depuis n'importe quel nom de réalisateur touchable (ci-dessous). Les séries (`type: "tv"`) sont retirées d'emblée, avant tout le reste : le propriétaire n'en a pas besoin ici, seulement des films et des courts métrages. Sans barre du bas, un `IconButton` retour en tête, **puis la bande de perforations de l'habillage « papier et pellicule » depuis la revue du 24 septembre 2026 (point 11) — absente de cette page avant elle, seule page du Voyage sans aucun de ses ornements** ; le `Scaffold` prend le fond du monde du Voyage (§2, phase 2) de l'année du **premier film daté** de la filmographie (séries déjà écartées), sinon 1895. Un seul défilement, `LazyVerticalGrid` (`GridCells.Fixed(3)`, gouttières 12 dp, marge 16 dp) : l'en-tête en est le tout premier item, en pleine largeur — centrés, la photo (`Portrait`, ronde, 96 dp), le nom (`titleLarge` espacé de 4 sp en gras, dans l'accent de ce monde, comme un titre de décennie), les dates s'il y en a (« 1861 – 1938 » inchangé, ou « né en 1958 »/« née en 1958 »/« naissance en 1958 » selon `genre`), **la biographie, vérifiée et masquée derrière un `TextButton` « Lire la biographie » quand elle arrive en anglais** (`biographieEstProbablementAnglaise`, fonction pure testée, revue du 24 septembre 2026, point 11 : le contrat dit lui-même n'envoyer du français que s'il en a, avec repli sur l'anglais sinon, `docs/openapi.json` — un accent français dans le texte tranche tout de suite pour le français, sinon au moins deux mots anglais très courants tranchent pour l'anglais ; avant cette revue, affichée d'office quelle que soit la langue), le bouton **« Suivre »**/**« Suivi »** (mêmes appels et même message que sur « Suivis »), puis un résumé (`bodySmall onSurfaceVariant`) « 42 films · 9 vus · 3 sur le Plex », accordé au singulier, « 0 vu »/« 0 sur le Plex » écrits quand même, compté sur les films et les courts métrages (les séries en sont déjà sorties), introuvables compris. Puis l'interrupteur « Masquer les introuvables » (`Switch`, activé par défaut, mémorisé pour la session — le même que sur la fiche d'une saga) : activé, les films que j'ai marqués introuvables sont absents de la grille, et une décennie qui n'avait qu'eux disparaît avec ; désactivé, ils reprennent leur place. Puis la filmographie, groupée par décennie dans l'ordre où le back la rend, un en-tête par groupe (« Années 1980 », `titleMedium` dans l'accent du monde de **cette** décennie, **suivi d'un filet de 2 dp dans ce même accent depuis la revue du 24 septembre 2026, point 11 — la couleur de la décennie devient un accent complet, pas seulement la teinte du texte comme avant elle**) : **une seule grille par décennie**, trois affiches par ligne, longs et courts métrages mêlés dans l'ordre du back (date de sortie croissante) — l'appli ne réordonne plus rien ; plus de ligne de repli « *N* courts · *N* série(s) », plus de grille à quatre, plus d'état déplié/replié (retouche du 22 septembre 2026, le propriétaire regardant dans l'ordre de sortie, décennie par décennie). Chaque affiche (vue en couleur avec sa pastille de note, sépia sinon, **le badge Plex unique de l'appli en coin (`PlexBadge`, `ui/Cover.kt`, une icône Tabler dans un petit cercle sombre — depuis la revue du 24 septembre 2026, point 2, qui en fait le seul badge Plex de l'appli ; avant elle, une icône `cloud` nue, sans fond)**, coin **« Court »** sur un court métrage — jumeau du coin « TV » qu'il remplace, même place, même style, même taille que les longs — liseré de 1,5 dp dans l'accent quand l'année est ouverte dans le Voyage) porte sous elle son titre (`bodyMedium`, deux lignes au plus) puis son année (`labelSmall onSurfaceVariant`) — lisibles même sans jaquette, `Cover` gardant alors son rectangle sépia. Toucher une affiche ouvre `Screen.FicheVoyage` si le film y a une ligne, sinon la fiche simple ci-dessous. Un appui long sur un non-vu ouvre la feuille « Marquer introuvable »/son inverse, comme sur la fiche d'une saga. |
