...
| **Fiche d'un film du Voyage** (`Screen.FicheVoyage`, nouvel écran, 21 septembre 2026) | Empilée depuis une affiche d'`AnneeScreen`, sans barre du bas, dans la palette du monde de l'année. De haut en bas : le fond héros (l'affiche du film, fondu dans le fond uni dès 40 % de sa hauteur depuis la revue du 24 septembre 2026, point 3), l'affiche (96×144, comme en tête du formulaire), le titre et le titre original s'il diffère (`titreOriginalAffiche`, fonction pure testée depuis cette même revue, point 4), le réalisateur, la durée totale s'il s'agit d'un programme ; **la salle s'affiche en étiquette « Salle · *nom* » (`labelLarge`, accent) depuis la revue du 24 septembre 2026 (point 9), la raison du film en simple texte sous elle si elle existe — avant cette revue, un cartouche papier jauni plein écran, une boîte quasi vide dès que la raison manquait** ; **ma note porte son libellé, « Ta note · *N* » (`titleMedium`, accent, depuis cette même revue — avant elle, un chiffre nu dans un cercle sans légende)** et mes réactions (emojis, `Reactions.emoji`) si je l'ai déjà vu, lues dans le journal chargé par `FriseViewModel` — relu après tout enregistrement réussi, quel que soit le film (correctif du 22 septembre 2026, « la fiche du Voyage se relit après un enregistrement »), pas seulement à l'ouverture de l'application ; l'état d'un film dans sa salle (vu, sur le Plex, demandé…) se relit de la même façon au retour du formulaire, même quand l'année était déjà prête ; pour un programme, la liste de ses bobines (affiche 40×60, titre, durée, état), chacune touchant le formulaire pré-rempli ; puis, selon l'état, jusqu'à cinq boutons : **« Voir sur le Plex »** (visible dès qu'un lien existe, quel que soit l'état — `plex_url` commence par `plex://` ou par `https://`, ouvert tel quel par un `Intent.ACTION_VIEW`, absent si nul), **« Je l'ai vu »** (disparaît une fois le film vu, seul cas où il n'a plus lieu d'être), **« Demander sur Sir »** (seulement sur « à demander », jamais sur « demandé »), **« Introuvable »** et son inverse **« Le remettre à voir »** (mutuellement exclusifs, ni l'un ni l'autre sur un film vu), **« Mettre sur le podium »** (brief du 21 septembre 2026, seulement sur un film ou un programme vu) qui ouvre la feuille **« Mettre sur le podium »** : trois lignes « Marche *N* · *occupant actuel* » ou « Marche *N* · libre », la marche qui porte déjà ce film ou ce programme cochée (✓ corail). **« Le film »** (décision 4 du brief du 24 septembre 2026, « le voyage revu ») ferme la liste : rouvre le carton dans une `FeuilleDeLecture` (`BoutonLeFilm`). |
...
| **Fiche simple d'un film** (`Screen.FicheFilm`, décision 2 du brief du 21 septembre 2026, « la page réalisateur ») | Ouverte au tap sur une affiche de la filmographie d'un réalisateur dont le film n'a pas de ligne dans le Voyage — nourrie par cette ligne de filmographie, rien d'autre à charger. **Le fond héros depuis la revue du 24 septembre 2026 (point 10)** : `backdrop_url` (`w1280`, dans le contrat depuis la v1.17.0 mais jamais lu par l'appli avant cette revue), fondu dans le fond uni dès 40 % de sa hauteur, comme le formulaire et la fiche d'un film du Voyage — **ni synopsis, ni durée, ni genres dans cette réponse** (`GET /me/realisateurs/{tmdbId}/page`, vérifié dans `contract/openapi.json`) : rien de plus à afficher que ce qui suit. En tête, retour, puis l'affiche (96×144, comme la fiche du Voyage), le titre et le titre original s'il diffère, l'année, le nom du réalisateur (touchable, ci-dessous), mon état (« Vu ») et ma note si je l'ai vu. Puis, selon l'état : **« Je l'ai vu »** (→ le formulaire pré-rempli, comme un à-voir du Plex), **« Voir sur le Plex »** (lien `plex://` puis web), **« Demander sur Sir »** (→ « demandé »), **« Introuvable »** — **en bouton texte gris depuis cette même revue, au lieu du corail par défaut de Material, réservé ailleurs à un choix ou un déclenchement** — et son inverse. Une **série** (`type: "tv"`) n'a ni « Je l'ai vu » ni formulaire : elle garde seulement Plex et Sir, avec une ligne « Les séries se suivent dans Suivis ». **« Le film »** (décision 4 du brief du 24 septembre 2026, « le voyage revu »), en dernier, rouvre le carton dans une `FeuilleDeLecture`. |
...
`realisateur/RealisateurRoutes.kt` et `profile/ProfileRoutes.kt` (le profil, SensCritique, le
rapport d'import). C'est là que vivent désormais les clés de `viewModel(key = …)` et les
...
FicheVoyageScreen.kt  nouvel écran (21 septembre 2026) : la fiche d'un film ou d'un
                       ...
                       à la chronique » et le carton inline
