...
| **Réalisateur** (`Screen.Realisateur`, brief du 21 septembre 2026, « la page réalisateur ») | Une seule page, suivi ou non, nourrie par `GET /me/realisateurs/{tmdbId}/page` — ouverte depuis « Suivis » ou depuis n'importe quel nom de réalisateur touchable (ci-dessous). Sans barre du bas : un `IconButton` retour en tête, puis, centrés, la photo (`Portrait`, ronde, 96 dp, initiale sur `surfaceContainerHigh` si TMDB n'en a pas), le nom (`titleLarge`), les dates s'il y en a (« 1861 – 1938 », ou « née en 1961 » selon ce que TMDB donne, absentes sinon), la présentation (`bodyMedium onSurfaceVariant`, première phrase de la biographie TMDB), puis le bouton **« Suivre »** (`FilledTonalButton`) ou **« Suivi »** (`OutlinedButton`, un tap le retire) — mêmes appels et même message que sur « Suivis ». Puis la filmographie complète, films et séries mêlés, chronologique, en étagère d'affiches (`LazyRow`, 72×108 dp, comme une salle du Voyage) : vus en couleur avec leur pastille de note, les autres en sépia ; un coin discret « Plex » (icône `Cloud`, 12 dp) quand le film est sur le Plex du propriétaire, un liseré de 1,5 dp dans l'accent du monde de l'année (§2, phase 2) quand l'année est ouverte dans le Voyage, un coin « TV » pour une série ; l'année sous chaque affiche. Toucher une affiche ouvre `Screen.FicheVoyage` si le film y a une ligne, sinon la fiche simple ci-dessous. Un appui long sur un non-vu ouvre la feuille « Marquer introuvable »/son inverse, comme sur la fiche d'une saga. |
...
AnneeViewModel.kt     réécrit le 21 septembre 2026 : les salles d'une année (GET
                       ...
                       correspond (mettreAJourSeanceFilm)
