...
| **Feuille « Lequel sur SensCritique ? »** (brief du 14 septembre 2026) | Un `ModalBottomSheet` : une ligne par candidat (affiche 56×84, titre, année, et réalisateur si un jour SensCritique le rend), puis un `TextButton` « Aucun de ceux-là » en bas, la liste bornée pour que ce bouton reste toujours visible même avec une dizaine de candidats. Le retour système la referme comme toute autre feuille (Material 3 ne s'y oppose pas) : ce geste équivaut à « Aucun de ceux-là » côté file, mais sans mémoriser de choix — la correction suivante du film redemandera (revue du 14 septembre 2026, critique 2). |
